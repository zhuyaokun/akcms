...
if(substr_count($_SERVER['SCRIPT_NAME'], '/') < 2) {
	aexit("<a href='http://www.akhtm.com/manual/197.htm'>{$lan['rootinstallerror']}</a>");
}
if(ifinstalled()) {
	adminmsg($lan['reinstallerror'], 'http://www.akhtm.com/manual/198.htm', 999, 1);
}
...
if(!empty($message)) aexit('<a href="http://www.akhtm.com/manual/196.htm" target="_blank">'.$message.'</a>');
$dbenv = checkdbenv();
$drivers = array(
