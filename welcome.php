...
if($get_action == 'phpinfo') {
	...
	$apiurl = 'http://www.akhtm.com/api/checknew2.php?ver='.$sysedition;
	$result = readfromurl($apiurl);
	$apiurl = 'http://www.akhtm.com/api/changelog4.php?ver='.$sysedition.'&akpath=http://'.$_SERVER['HTTP_HOST'].substr(AK_URL, 0, -1);
	if($result == '1') {
		aexit("$('#newversion').html('<a href=\"$apiurl\" target=\"_self\">{$lan['findnew']}</a>');");
	} else {
		aexit("$('#newversion').html('<span class=\'green\'>{$lan['alreadylatest']}<\/span>');");
	}
